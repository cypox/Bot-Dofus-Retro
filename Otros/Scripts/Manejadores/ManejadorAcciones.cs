        ...
        public void get_Funcion_Personalizada(DynValue coroutine)
        {
            ...
            if (account.hasGroup && account.isGroupLeader)
            {
                Task.Factory.StartNew(async () =>
                {
                    ...
                    {
                        await Task.Delay(500);
                        account.Logger.LogInfo("CUSTOM", "EXECUTING FOR MEMBER " + groupMember.game.character.nombre);
                        groupMember.script.actions_manager.coroutine_actual = manejador_script.script.CreateCoroutine(coroutine);
                        groupMember.script.actions_manager.procesar_Coroutine();
                    }
                });
            }
            //*/
